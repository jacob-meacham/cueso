...
class TestOpenRouterBaseUrl:
    """The client targets OpenRouter by default; base_url stays overridable
    for other OpenAI-compatible endpoints."""

    def test_default_targets_openrouter(self) -> None:
    ...
    def test_base_url_override(self) -> None:
        provider = OpenRouterProvider(api_key="test-key", model="test-model", base_url="https://example.com/v1")
        assert str(provider.client.base_url).rstrip("/") == "https://example.com/v1"
